...
main()
{
    ...
    int i;
	int result;	
	char param_country[35]; //country name as key
	char input_filename[50]; // input
	char output_filename[50]; // output
...
-------------------------------------------------------------------*/
	...
	for (i = 0; i < 5; i++) {
		...
		if (result) {
			printf("%s  ", Data.Country_Name);  /* TO DO: print country name. */
			printf("%ld ", Data.Population);  /* TO DO: print population. */ 
			printf("%.2f ", Data.Life_Expectancy);  /* TO DO: print life_expectancy */
			printf("%s %d %d\n", Data.Country_Daily_Data[Data.Number_of_Daily_Data - 1].Date, 
				Data.Country_Daily_Data[Data.Number_of_Daily_Data - 1].New_Cases, 
				Data.Country_Daily_Data[Data.Number_of_Daily_Data - 1].New_Deaths); /* TO DO: print the last row of data, i.e, date, new_cases and new_deaths. */
		}
	}
	printf("\n\n");	
